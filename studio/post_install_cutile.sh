...
BRANCH="${UNSLOTH_CUTILE_BRANCH:-main}"
...
Options:
  ...
  --branch REF
      ...
      Default: main, or UNSLOTH_CUTILE_BRANCH if set.

  --unsloth-repo-url URL
      Git URL used for fallback Unsloth installs when --unsloth-root does not
      point at a local source tree. Default:
        https://github.com/unslothai/unsloth.git

  --zoo-repo-url URL
      Git URL used for fallback Unsloth Zoo installs when --zoo-root does not
      point at a local source tree. Default:
        https://github.com/unslothai/unsloth-zoo.git
...
Examples:
  ...
  ./studio/post_install_cutile.sh --branch feat/cutile \
    --unsloth-repo-url https://github.com/mmathew23/unsloth.git \
    --zoo-repo-url https://github.com/mmathew23/unsloth-zoo.git
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --branch)
            ...
            ;;
        --unsloth-repo-url)
            [ "$#" -ge 2 ] || die "--unsloth-repo-url requires an argument"
            UNSLOTH_REPO_URL="$2"
            shift 2
            ;;
        --zoo-repo-url)
            [ "$#" -ge 2 ] || die "--zoo-repo-url requires an argument"
            ZOO_REPO_URL="$2"
            shift 2
            ;;
        --cutile-spec)
...
if [ -n "$ZOO_ROOT" ]; then
    ...
    log "Unsloth Zoo root: none found; using $ZOO_REPO_URL at ref $BRANCH"
fi
...
if [ -f "$UNSLOTH_ROOT/pyproject.toml" ]; then
    ...
    log "Installing unsloth[$UNSLOTH_EXTRA] from $UNSLOTH_REPO_URL at ref $BRANCH"
    run_cmd "${PIP[@]}" install --force-reinstall --no-deps \
        "unsloth[$UNSLOTH_EXTRA] @ git+$UNSLOTH_REPO_URL@$BRANCH"
fi
