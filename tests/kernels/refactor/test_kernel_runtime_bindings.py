...
""",
    ...
    def test_runtime_function_clones_get_distinct_code_objects(self):
        def _impl(x):
            return helper(x)
        ...
        clone_a = _clone_function_with_globals(_impl, {"helper": _helper_a})
        clone_b = _clone_function_with_globals(_impl, {"helper": _helper_b})

        self.assertIsNot(clone_a.__code__, clone_b.__code__)
        self.assertNotEqual(clone_a.__code__.co_name, clone_b.__code__.co_name)
        self.assertEqual(clone_a(torch.tensor(1)).item(), 2)
        self.assertEqual(clone_b(torch.tensor(1)).item(), 3)
