...
try:
    ...
    if "unsloth_zoo" in sys.modules:
        ...
        if other_zoo_file:
            ...
            if other_zoo_folder != _local_zoo_folder:
                warnings.warn(
                    f"Unsloth: A different copy of 'unsloth_zoo' was imported and does not match the expected location {other_zoo_folder} != {_local_zoo_folder}. "
                    f"Overriding to use {_local_zoo_folder}."
                )
    ...
    raise ImportError(
        f"Unsloth: There was an error importing unsloth_zoo. "
        f"Please check the environment variable UNSLOTH_ZOO_MODULE and make sure it is set to the correct module path. "
        f"{os.getenv('UNSLOTH_ZOO_MODULE')}"
    )
pass
